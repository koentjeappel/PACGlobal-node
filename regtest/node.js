...
describe('Node Functionality', function() {
  ...
  before(function(done) {
    ...
    testKey = pacglobal.PrivateKey(testWIF);
    ...
    rimraf(datadir + '/regtest', function(err) {
        ...
        services: [
          {
            ...
            config: {
              spawn: {
                datadir: datadir,
                exec: path.resolve(__dirname, process.env.HOME, './.PACGlobal/data/dashd')
              }
            }
          }
        ]
      ...
      regtest = pacglobal.Networks.get('regtest');
  ...
  describe('Address Functionality', function() {
    ...
    describe('History', function() {
      ...
      before(function(done) {
        ...
        testKey2 = pacglobal.PrivateKey.fromWIF('cNfF4jXiLHQnFRsxaJyr2YSGcmtNYvxQYSakNhuDGxpkSzAwn95x');
        address2 = testKey2.toAddress(regtest).toString();

        testKey3 = pacglobal.PrivateKey.fromWIF('cVTYQbaFNetiZcvxzXcVMin89uMLC43pEBMy2etgZHbPPxH5obYt');
        address3 = testKey3.toAddress(regtest).toString();

        testKey4 = pacglobal.PrivateKey.fromWIF('cPNQmfE31H2oCUFqaHpfSqjDibkt7XoT2vydLJLDHNTvcddCesGw');
        address4 = testKey4.toAddress(regtest).toString();

        testKey5 = pacglobal.PrivateKey.fromWIF('cVrzm9gCmnzwEVMGeCxY6xLVPdG3XWW97kwkFH3H3v722nb99QBF');
        address5 = testKey5.toAddress(regtest).toString();

        testKey6 = pacglobal.PrivateKey.fromWIF('cPfMesNR2gsQEK69a6xe7qE44CZEZavgMUak5hQ74XDgsRmmGBYF');
    ...
    describe('Mempool Index', function() {
      ...
      it('will update the mempool index after new tx', function(done) {
        var memAddress = pacglobal.PrivateKey().toAddress(node.network).toString();
        var tx = new Transaction();
        tx.from(unspentOutput);
