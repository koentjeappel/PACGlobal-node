...
/**
 * Provides a friendly event driven API to pacglobald in Node.js. Manages starting and
 * stopping pacglobald as a child process for application support, as well as connecting
 * to multiple pacglobald processes for server infrastructure. Results are cached in an
...
Dash.prototype.subscribeAddress = function(emitter, addresses) {
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (pacglobal.Address.isValid(addresses[i], this.node.network)) {
...
Dash.prototype._loadSpawnConfiguration = function(node) {
  ...
  var configPath = path.resolve(spawnOptions.datadir, './pacglobal.conf');

  log.info('Using Pacglobal config file:', configPath);
...
Dash.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of pacglobal-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of pacglobal-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of pacglobal-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to pacglobald from pacglobal. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that dashd is undergoing a reindex. ' +
             ...
             'of pacglobal-node services will start.');
    node._reindex = true;
  }
};
...
Dash.prototype._getDefaultConf = function() {
  ...
  if (this.node.network === pacglobal.Networks.testnet) {
...
Dash.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = pacglobal.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    var txid = pacglobal.util.buffer.reverse(hash).toString('hex');
...
Dash.prototype._stopSpawnedDash = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/pacglobald.pid';
...
Dash.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedDash(function(err) {
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn pacglobald.'));
      }
...
Dash.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to pacglobald.'));
    }
 ...
 * @param {String|Address|Array} addressArg - An address string, pacglobal address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
...
Dash.prototype.getAddressUnspentOutputsPaginated = function(addressArg, options, callback) {
  ...
  function transformUnspentOutput(delta) {
    var script = pacglobal.Script.fromAddress(delta.address);
    return {
...
Dash.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    ...
    if (clone) {
      // reverse start and end as the order in pacglobal is most recent to less recent
...
Dash.prototype._getAddressStrings = function(addresses) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof pacglobal.Address) {
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = pacglobal.Block.fromString(response.result);
  ...
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('pacglobald spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('pacglobald process did not exit'));
      }
