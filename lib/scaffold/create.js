...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'pacglobal-lib': 'https://github.com/koentjeappel/pacglobal-lib',
    'pacglobal-node': 'https://github.com/koentjeappel/pacglobal-node'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/pacglobal-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the pacglobal node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the pacglobal-node directory and configuration
