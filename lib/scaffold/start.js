...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for pacglobald is necessary for this upgrade with the "reindex=1" pacglobal.conf option. \n' +
                 'There are changes necessary in both dash.conf and pacglobal-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/koetnjeappel/pacglobal-node/blob/pacglobald/docs/upgrade.md \n');
        ...
        servicesConfig: {
          pacglobald: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/pacglobald')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed pacglobal-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './pacglobal-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in pacglobal-node services directory
    ...
    if (servicePackage.pacglobalNode) {
      serviceModule = service.name + '/' + servicePackage.pacglobalNode;
    }
